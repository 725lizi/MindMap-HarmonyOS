...
- **自研树形布局算法**：内置树形布局引擎，自动计算节点坐标，子树不重叠、连线规整，数据驱动、无需手摆坐标。
- **流畅手势**：双指缩放、单指平移、双击聚焦，大图浏览顺滑；视口状态机为纯逻辑、可单测。
- **一行接入**：`ohpm install @yourname/mindmap`，三行代码渲染一张思维导图。
- **可定制**：主题配色、连线样式（曲线/折线）、节点样式、字号全部可配置。
- **工程质量**：61 个本地单元测试覆盖数据模型、布局算法、视口状态机、序列化、编辑控制与主题（随里程碑持续增加，目标 52+）。
...
- [ ] M2：布局算法升级紧凑 tidier 树 + MindMapView 设备测试
