...
- **layout**：输入 MindNode 树，输出 `LayoutNode[]`（坐标）+ `Edge[]`（连线锚点）+ 内容包围盒；不碰 UI。
  - V0.1 曾用「递归分层布局（简单但正确）」保底：x 由深度决定、叶子纵向占槽、父节点居中于首尾子节点。
  - M2 已升级为后序轮廓紧凑 tidier 布局：后序遍历返回每棵子树按相对深度的轮廓，新兄弟子树只在轮廓相撞的深度整体下移最小距离，父节点居中于首末子节点，不对称树更紧凑；输入输出契约不变，渲染层无感。
